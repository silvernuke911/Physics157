...
p_1 = Particle(6, np.array([0,0]), np.array([1,2]))
p_2 = Particle(4, np.array([0,-1.5]), np.array([-2,0]))
p_3 = Particle(5, np.array([-1,0]), np.array([0,-1]))
...
a1[0] = (grav_force(p_1.mass,p_2.mass, r1[0], r2[0])+grav_force(p_1.mass,p_2.mass, r1[0], r3[0])) / p_1.mass
a2[0] = (grav_force(p_1.mass,p_2.mass, r2[0], r1[0])+grav_force(p_1.mass,p_2.mass, r2[0], r3[0])) / p_2.mass
a3[0] = (grav_force(p_1.mass,p_2.mass, r3[0], r1[0])+grav_force(p_1.mass,p_2.mass, r3[0], r2[0])) / p_3.mass

time_start = time.time()

for i in range(1,len(t)):

    a1[i] = (grav_force(p_1.mass,p_2.mass, r1[i-1], r2[i-1]) + grav_force(p_1.mass,p_3.mass, r1[i-1], r3[i-1])) / p_1.mass
    a2[i] = (grav_force(p_2.mass,p_1.mass, r2[i-1], r1[i-1]) + grav_force(p_2.mass,p_3.mass, r2[i-1], r3[i-1])) / p_2.mass
    a3[i] = (grav_force(p_3.mass,p_1.mass, r3[i-1], r1[i-1]) + grav_force(p_3.mass,p_2.mass, r3[i-1], r2[i-1])) / p_3.mass
...
plt.plot(r1[:,0],r1[:,1],marker='',zorder=1,color='red')
plt.plot(r2[:,0],r2[:,1],marker='',zorder=1,color='blue')
plt.plot(r3[:,0],r3[:,1],marker='',zorder=1,color='green')

plt.scatter(r1[0][0],r1[0][1],marker='.',s=60,color='red',zorder=2)
plt.scatter(r2[0][0],r2[0][1],marker='.',s=60,color='blue',zorder=2)
plt.scatter(r3[0][0],r3[0][1],marker='.',s=60,color='green',zorder=2)
...
def animated_plot():
    ...
    with writer.saving(fig, filename , 200):
        for i in range(0,len(t),40):
            ...
            plt.scatter(r3[i][0],r3[i][1],marker='.',s=60,color='green',zorder=2)

            plt.yticks(fontsize=12)
            plt.xticks(fontsize=12)
            plt.xlabel('$x$-axis (AU)')
            plt.ylabel('$y$-axis (AU)')
            plt.title(f'Three body problem simulation : t = {(t[i] / year ):.3f} years')
